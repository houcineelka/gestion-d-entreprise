...
class HistoriqueController {
    ...
    private function loadHistorique() {
        try {
            ...
            $countSql = "SELECT COUNT(*) as total FROM audit h WHERE 1=1";
    ...
    private function loadStatistiques() {
        try {
            // Total des actions
            $sql = "SELECT COUNT(*) as total FROM audit";
            $stmt = $this->db->query($sql);
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
            $this->statistiques['total_actions'] = $result ? $result['total'] : 0;

            // Actions aujourd'hui
            $sql = "SELECT COUNT(*) as total FROM audit WHERE DATE(date_action) = CURDATE()";
            $stmt = $this->db->query($sql);
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
            $this->statistiques['today_actions'] = $result ? $result['total'] : 0;

            // Actions cette semaine
            $sql = "SELECT COUNT(*) as total FROM audit WHERE YEARWEEK(date_action, 1) = YEARWEEK(CURDATE(), 1)";
            $stmt = $this->db->query($sql);
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
            $this->statistiques['week_actions'] = $result ? $result['total'] : 0;

            // Actions ce mois
            $sql = "SELECT COUNT(*) as total FROM audit WHERE YEAR(date_action) = YEAR(CURDATE()) AND MONTH(date_action) = MONTH(CURDATE())";
            $stmt = $this->db->query($sql);
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
